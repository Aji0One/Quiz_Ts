...
const Tfq = ({ id, num, setrender }: questionProps) => {
        ...
        <div className='quiz-cont' >
            ...
            {currentQuestion?.opt.map((ele: any, i: any) => (
                <div key={i} className='options'>
                    <input type="radio" name='tf' value={ele.op} onChange={() => handleAnswer(ele.op)} checked={myans?.anser && myans?.anser[0] === ele.op} className='input' data-testid={`o-${i}`} />
